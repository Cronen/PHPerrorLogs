...
function pro_expand(obj, tbl_id)
{
    ...
    if (obj.attr('data-action') == "visible")
    {
        obj.attr('data-action', 'hidden');
        $(".stack-trace-row" + tbl_id).hide();
        return;
    } else
    {
        obj.attr('data-action', 'visible');
    }
    ...
    //fadedown after buttonclick med sleep 
    $(".stack-trace-row" + tbl_id).slideDown('slow', function () {
...
function pro_postpone(obj)
{
    var action = "pro_postpone";
    $("#postponeModal").load('/includes/ajax/pro_handling_ajax.php?', {'action': action});
